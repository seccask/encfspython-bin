...
def processCriteoAdData(d_path, d_file, npzfile, split, convertDicts, pre_comp_counts, total_per_file):
    ...
    for i in range(split):
        for s in range(math.ceil(total_per_file[i] / NUM_DATA_IN_SPLIT)):
            start_time = time.time()
            
            filename_i = npzfile + "_{0}_split_{1}_processed.npz".format(i, s)
            if path.exists(filename_i):
                print("Using existing " + filename_i, end="\r")
            else:
                with np.load(npzfile + "_{0}_split_{1}.npz".format(i, s)) as data:
...
def concatCriteoAdData(
    ...
    if memory_map:
        ...
        for i in range(days):
            for s in range(math.ceil(total_per_file / NUM_DATA_IN_SPLIT)):
                filename_i = npzfile + "_{0}_split_{1}_processed.npz".format(i, s)
                with np.load(filename_i) as data:
                    if i == 0:
                        X_cat = data["X_cat"]
                ...
                print("Loaded day:", i, "split:", s, "y = 1:", len(y[y == 1]), "y = 0:", len(y[y == 0]))
...
def getCriteoAdData(
    ...
    if path.exists(total_file):
        ...
        if criteo_kaggle:
            ...
            for i in range(days):
                ...
                print("Try path=%s" % (str(datafile_i)))
                if path.exists(str(datafile_i)):
                    ...
                    with open(str(datafile_i)) as f:
                        for _ in f:
                            ...
                            ### DEBUG OPTION ###
                            # if total_per_file_count >= 1000000:
                            #     break
    ...
    def process_one_file(
        ...
        #         convertDicts[j][x] = 1
        # store parsed
        filename_s = npzfile + "_{0}_split_{1}.npz".format(day, split)
        if path.exists(filename_s):
            print("\nSkip existing " + filename_s)
        else:
            with open(filename_s, "wb") as wf:
                np.savez_compressed(
                    wf,
                    X_int=X_int[0:i, :],
                    # X_cat=X_cat[0:i, :],
            ...
            print("\nSaved " + npzfile + "_{0}_split_{1}.npz!".format(day, split))
        return i
    ...
    for i in range(days):
        ...
        if path.exists(npzfile_i):
            ...
            recreate_flag = True
            f = open(str(datfile_i))
            num_splits = math.ceil(total_per_file[i] / NUM_DATA_IN_SPLIT)
            total_per_file[i] = 0
    ...
    # process all splits
    processCriteoAdData(d_path, d_file, npzfile, days, convertDicts, counts, total_per_file)
...
if __name__ == "__main__":
    ...
    print('Total Time: %f' % (time.time() - time1))
