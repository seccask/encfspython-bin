    ...
    for data_set in sets:
        ...
        with open(file_list % data_set) as f:
            for img_id, img_name in enumerate(f):
                ...
                imageio.imwrite(os.path.join(data_dir, img_name + '.png'), labelMap)
                # scipy.misc.imsave(
        ...
        with open(os.path.join(out_dir, json_name % data_set), 'ab') as outfile:
...
def convert_cityscapes_instance_only(
    ...
    for data_set, ann_dir in zip(sets, ann_dirs):
        ...
        with open(os.path.join(out_dir, json_name % data_set), 'a') as outfile:
            outfile.write(json.dumps(ann_dict))
