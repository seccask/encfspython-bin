...
class Preprocessor:
    ...
    def preprocess_dataset(self):
        ...
        cases = sorted([f for f in os.listdir(self.data_dir) if "case" in f])
        for i, case in enumerate(cases):
            ...
            print('[%d/%d] Duration: %f' % (i + 1, len(cases), time.time() - start_time))
    ...
    def resample3d(self, image, label, image_spacings):
        if image_spacings != self.target_spacing:
            ...
            image = interpolate(torch.as_tensor(np.expand_dims(image, 0), dtype=torch.float32),
                                size=new_shape, mode='trilinear', align_corners=True)
            label = interpolate(torch.as_tensor(np.expand_dims(label, 0), dtype=torch.float32), size=new_shape, mode='nearest')
